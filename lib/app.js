...
module.exports = (app, options) => {
  ...
  try {
    ...
    const renderApp = (req, res) => {
      ...
      renderer.renderToString(context, (err, html) => {
        if (err) {
          ...
          if (cacheable) {
            cache.set(req.url + getCookieServer('darkMode', req.headers.cookie), html)
          }
    ...
    app.get('*', (req, res, next) => {
      if (req.useragent.isMobile && (req.url == '/' || req.url.startsWith('/?'))) {
        return res.redirect('/m/home/' + req.url.slice(1));
      } else if (!req.useragent.isMobile && (req.url == '/m/home' || req.url.startsWith('/m/home/?'))) {
      ...
      const cacheable = isCacheable(req)
      if (cacheable) {
        const hit = cache.get(req.url + getCookieServer('darkMode', req.headers.cookie))
        if (hit) {
