...
module.exports = (app, options) => {
  ...
  global.enableApolloCache = process.env.ENABLE_APOLLO_CACHE
  options = Object.assign({}, DEFAULT_OPTIONS, options)
  const gauge = new client.Gauge({ name: 'request_duration', help: 'time elapsed during ssr' });
  ...
  try {
    ...
    // Serve static files
    const serve = (filePath) => express.static(filePath, {
      index: false,
    })
    ...
    const renderApp = (req, res, startTime) => {
      ...
      renderer.renderToString(context, (err, html) => {
        if (err) {
          ...
          if (cacheable && process.env.ENABLE_NODEJS_CACHE) {
            // cache.set(getCacheKey(req), html)
          }
          // console.log('from express');
          gauge.set(new Date().getTime() - startTime)
    ...
    if (isProd) {
      ...
      // In development: wait for webpack compilation
      // when receiving a SSR request
      ssr = (req, res, startTime) => {
        readyPromise.then(() => renderApp(req, res, startTime)).catch(console.error)
      }
    }
    ...
    app.get('/prometheus/metrics', function (req, res) {
    ...
    app.get('*', (req, res, next) => {
      ...
      console.log(os.freemem(), req.url);
      if (config.skipRequests(req)) {
        return next()
      }

      const cacheable = isCacheable(req)
      if (cacheable && process.env.ENABLE_NODEJS_CACHE === 'true') {
