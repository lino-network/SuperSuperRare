...
module.exports = (app, options) => {
  ...
  try {
    ...
    const renderApp = (req, res) => {
      ...
      renderer.renderToString(context, (err, html) => {
        if (err) {
          ...
          if (cacheable) {
            cache.set(req.url + getCookieServer('darkMode', context.req.headers.cookie), html)
          }
    ...
    app.get('*', (req, res, next) => {
      ...
      const cacheable = isCacheable(req)
      if (cacheable) {
        const hit = cache.get(req.url + getCookieServer('darkMode', context.req.headers.cookie))
        if (hit) {
