...
Sentry.init({ dsn: 'https://752f65755df346989e7e544ef787707f@sentry.io/1402448' });
...
module.exports = (app, options) => {
  ...
  try {
    ...
    const renderApp = (req, res) => {
      ...
      renderer.renderToString(context, (err, html) => {
        if (err) {
          ...
          if (cacheable) {
            cache.set(req.url + getCookieServer('darkMode', req.headers.cookie) + (req.headers['cloudfront-viewer-country'] && req.headers['cloudfront-viewer-country'][0].value) || '', html)
          }
    ...
    app.get('/setDarkMode', function(req, res) {
      res.cookie('darkMode', req.query.darkMode, { maxAge: 2592000000, httpOnly: true });
      res.send(200);
    })

    app.get('*', (req, res, next) => {
      // console.log('========', req.useragent.isMobile, req.headers['cloudfront-is-mobile-viewer'])
      console.log(req.headers['cloudfront-is-mobile-viewer'], typeof req.headers['cloudfront-is-mobile-viewer']);
      if (req.useragent) {
        console.log(req.useragent.isMobile)
      }
      if ((req.headers['cloudfront-is-mobile-viewer'] == 'true' || (req.useragent && req.useragent.isMobile)) && (req.path == '/' )) {
        res.setHeader('Cache-Control', 'public, max-age=0, s-maxage=20')
        return res.redirect('/m/home/' + req.url.slice(1));
      } else if ((req.headers['cloudfront-is-mobile-viewer'] == 'false' || (req.useragent && !req.useragent.isMobile)) && (req.path == '/m/home/')) {
        res.setHeader('Cache-Control', 'public, max-age=0, s-maxage=20')
      ...
      if (cacheable) {
        ...
        const hit = cache.get(req.url + getCookieServer('darkMode', req.headers.cookie) + (req.headers['cloudfront-viewer-country']) || '')
        if (hit) {
