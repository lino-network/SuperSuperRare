...
module.exports = (app, options) => {
  ...
  const histogram = new client.Histogram({
    name: 'request_duation',
    help: 'time elapsed during ssr',
    labelNames: ['route'],
  ...
  try {
    ...
    const renderApp = (req, res, end, startTime) => {
      ...
      renderer.renderToString(context, (err, html) => {
        if (err) {
          ...
          histogram.labels(req.route.path).observe(new Date().getTime() - startTime)
    ...
    if (isProd) {
      ...
      // In development: wait for webpack compilation
      // when receiving a SSR request
      ssr = (req, res, end, startTime) => {
        readyPromise.then(() => renderApp(req, res, end, startTime)).catch(console.error)
      }
    }
    ...
    app.get('*', (req, res, next) => {
      ...
      const cacheable = isCacheable(req)
      console.log('enable nodejs cache status: ', process.env.ENABLE_NODEJS_CACHE)
