...
exports.changeEntryPoint = () => {
  const service = config.service
  let webpackConfig = service.resolveChainableWebpackConfig()
  webpackConfig = service.resolveWebpackConfig(webpackConfig)
  webpackConfig.output.path = path.resolve('dists')
  for (let i of webpackConfig.plugins) {
    if (i.constructor.name === 'HtmlWebpackPlugin') {
      i.options.filename = config.simpleTemplateName
      i.options.template = path.resolve('public/indexs.html')
    }
  }
  webpackConfig = mergeWebpack(webpackConfig, {
    entry: config.simpleEntry,
    plugins: [
...
exports.getWebpackConfig = ({ target }, isComplete = false) => {
  const service = config.service
  const isProd = process.env.NODE_ENV === 'production'
  const completeSourceMap = process.env.VUE_APP_SOURCE_MAP === 'complete'
  ...
  let smOptions = '';
  if (!isProd) {
    smOptions = '#cheap-module-source-map';
  } else {
    if (isComplete) { 
      smOptions = 'source-map'
    } else {
      smOptions = 'none'
    }
  }
