...
exports.getWebpackConfig = ({ target, page }) => {
  ...
  webpackConfig = mergeWebpack(webpackConfig, {
    entry: config.entry(target, page),
    plugins: [
    ...
    webpackConfig = mergeWebpack(webpackConfig, {
      plugins: [
        new VueSSRClientPlugin(
          {filename: `js/${page}/vue-ssr-client-manifest.json`}
        ),
        new WebpackBar({
          name: 'Client',
          color: 'green',
      ...
      devtool: !isProd ? '#cheap-module-source-map' : '#nosources-source-map',
    })
  } else {
    webpackConfig = mergeWebpack(webpackConfig, {
      plugins: [
        new VueSSRServerPlugin(
          {filename: `js/${page}/vue-ssr-server-bundle.json`}
        ),
        new WebpackBar({
          name: 'Server',
          color: 'orange',
